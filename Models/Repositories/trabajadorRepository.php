    ...
    private $pdo;
        ...
        public function obtenerTotales():array{
            ...
            try {
                ...
                $personalPorSede = $stmtPersonalPorSede->fetchAll(PDO::FETCH_ASSOC); 
        ...
        public function comparativaIngresos(): array{
            $sql = "SELECT
                        YEAR(horaRegistro) AS ano,                 -- Extrae el año (e.g., 2023, 2024)
                        MONTH(horaRegistro) AS mes,          -- Extraer el mes (e.g., 1 for January, 2 for February)
                        DATE_FORMAT(horaRegistro, '%Y-%m') AS anoFormato, -- Formato como 'YYYY-MM' (e.g., '2023-01') para facil clasificacion/etiquetado
                        DATE_FORMAT(horaRegistro, '%M') AS nombreMes,   -- Obtener el nombre completo del mes (e.g., 'January', 'February')
                        COUNT(*) AS total_personas                    -- cuanta la cantidad total de trabajadores por mes
                    FROM
                        tabla_personal
                    GROUP BY
                        ano,
                        anoFormato,
                        nombreMes                                    -- Agrupar por año y mes para mostrar resultado por mes
                    ORDER BY
                        ano ASC,
                        mes ASC                               -- Ordenar cronologicamente
                        ";
            try{
                $stmt = $this->pdo->prepare($sql);
                $stmt->execute();
        ...
        public function guardar(Trabajador $trabajador): ?Trabajador {
                ...
                try {
                    ...
                    $stmtDireccion->bindValue(':cedula', $trabajadorIdForRelations);
                    ...
                    $parientes = $trabajador->parientes ?? []; 

                    foreach ($parientes as $pariente) {
                        
                        $stmtPariente->bindValue(':trabajadorId', $trabajadorIdForRelations); 
                        $stmtPariente->bindValue(':cedula', $pariente['cedulaPariente'] ?? null); 
                        $stmtPariente->bindValue(':nombre', $pariente['nombrePariente'] ?? null);
                        $stmtPariente->bindValue(':apellido', $pariente['apellidoPariente'] ?? null);
                        $stmtPariente->bindValue(':fechaNacimiento', $pariente['fechaNacimientoPariente'] ?? null); 
                        $stmtPariente->bindValue(':parentesco', $pariente['parentesco'] ?? null);
                        $stmtPariente->bindValue(':genero', $pariente['generoPariente'] ?? null); 
            ...
            try {
                ...
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { 
                    $trabajadores[] = new Trabajador(
                        ...
                        cargoDirector: $row['nombre_cargoD'] 
                    );
                }
